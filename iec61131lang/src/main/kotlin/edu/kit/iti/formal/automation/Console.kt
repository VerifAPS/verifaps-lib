    ...
    val formatter = object : Formatter() {
        ...
        override fun format(record: LogRecord): String =
                "[%10.3f] [%6s] %s%n".format(
                        ...
                        when (record.level) {
                            Level.FINE -> colorizeFg(ConsoleColor.Cyan, record.level.name)
                            Level.FINER -> colorizeFg(ConsoleColor.Cyan, record.level.name)
                            Level.FINEST -> colorizeFg(ConsoleColor.Cyan, record.level.name)
                            Level.INFO -> colorizeFg(ConsoleColor.Blue, record.level.name)
                            Level.WARNING -> colorizeFg(ConsoleColor.Yellow, record.level.name)
                            Level.SEVERE -> colorize(ConsoleColor.Red, ConsoleColor.White, record.level.name)
