...
object SMVFacade {
    ...
    @JvmStatic
    fun next(expr: SMVExpr): SMVExpr = SFunction("next", expr)
    ...
    @JvmStatic
    fun combine(op: SBinaryOperator, vararg exprs: SMVExpr): SMVExpr = Arrays.stream(exprs).reduce(reducer(op)).get()

    @JvmStatic
    fun combine(op: SBinaryOperator, exprs: List<SMVExpr>): SMVExpr = exprs.stream().reduce(reducer(op)).get()

    @JvmStatic
    fun combine(op: SBinaryOperator, exprs: List<SMVExpr>, defaultValue: SMVExpr): SMVExpr = exprs.stream().reduce(reducer(op)).orElse(defaultValue)

    @JvmStatic
    fun reducer(op: SBinaryOperator): BinaryOperator<SMVExpr> = BinaryOperator({ a: SMVExpr, b: SMVExpr -> SBinaryExpression(a, op, b) })
    ...
    @JvmStatic
    fun NOT(e: SMVExpr): SUnaryExpression = SUnaryExpression(SUnaryOperator.NEGATE, e)
...
*/
    ...
    fun getParser(stream: CharStream): SMVParser = SMVParser(CommonTokenStream(SMVLexer(stream)))
