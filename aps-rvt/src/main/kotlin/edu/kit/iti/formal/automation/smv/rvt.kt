...
import edu.kit.iti.formal.smv.SMVModuleType
...
open class RegressionVerification(
    ...
    var oldModuleType: SMVModuleType? = null
    var newModuleType: SMVModuleType? = null
    ...
    protected open fun staticInitModule() {
        ...
        newModuleType = SMVModuleType(newVersion.name, newVersion.moduleParameters)
        oldModuleType = SMVModuleType(oldVersion.name, oldVersion.moduleParameters)
    }
    ...
    open fun writeTo(writer: Writer) {
        with(writer) {
            this.write(edu.kit.iti.formal.smv.SMVPrinter.toString(glueModule))
            val sep = "-".repeat(80) + "\n"
            this.write(sep)
            this.write(edu.kit.iti.formal.smv.SMVPrinter.toString(oldVersion))
            this.write(sep)
            this.write(edu.kit.iti.formal.smv.SMVPrinter.toString(newVersion))
            this.write(sep)
        }
    }
}
...
abstract class Miter(val oldVersion: SMVModule, val newVersion: SMVModule) {
    ...
    abstract fun build()
    open fun writeTo(writer: Writer) {
        writer.write(SMVPrinter.toString(module))
    }
}
...
open class ReVeWithMiter(oldVersion: SMVModule, newVersion: SMVModule, val miter: Miter)
    ...
    override fun addProofObligation(oldName: String, newName: String) {
        ...
        val miterVar = SVariable.create(miterInstanceName).with(
                SMVModuleType(miter.module.name, parameters)
        )
...
open class GloballyMiter(oldVersion: SMVModule, newVersion: SMVModule)
    ...
    override fun build() {
        ...
        for ((fst, snd) in output) {
            ...
            module.definitions.add(SAssignment(eq, old.equal(new)))
            list.add(eq)
        }
...
open class UntilMiter(oldVersion: SMVModule, newVersion: SMVModule, val inner: Miter)
    ...
    open fun event(expr: SMVExpr) {
        module.definitions.add(SAssignment(triggerCondition, expr))
    }
    ...
    override fun build() {
        ...
        module.stateVars.add(
                SVariable.create("inner")
                        .with(SMVModuleType(inner.module.name,
