...
public class Inheritance extends STOOTransformation {
    ...
    private static Pair<ClassDeclaration, Integer> getInheritedFrom(@NotNull ClassDeclaration accessedVariableClass,
        ...
        while (parentClass != null) {
            if (parentClass.getLocalScope().hasVariable(accessedField) || parentClass.hasMethod(accessedField))
                break;
    ...
    public void transform(@NotNull STOOSimplifier.State state) {
        ...
        for (ClassDeclaration classDeclaration : state.getGlobalScope().getClasses()) {
            ...
            List<String> classFields = classDeclaration.getEffectiveLocalScope().stream()
                            .filter(v -> !classDeclaration.getLocalScope().hasVariable(v.getName()))
    ...
    private class SetSuperAccessEffectiveTypeVisitor extends AstVisitor {
        ...
        public Object visit(@NotNull SymbolicReference symbolicReference) {
            if (((symbolicReference.getEffectiveDataType() instanceof ClassDataType
                ...
                symbolicReference.setEffectiveDataType(state.getGlobalScope().resolveDataType(inheritedFromClass));
            }
