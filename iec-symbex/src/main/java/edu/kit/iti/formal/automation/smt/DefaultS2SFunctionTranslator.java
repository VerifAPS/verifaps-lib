...
public class DefaultS2SFunctionTranslator implements S2SFunctionTranslator {
    ...
    @Override
    @Nonnull
    public Sexp translateOperator(@Nonnull SBinaryOperator operator,
                                  @Nonnull SMVType typeLeft,
                                  @Nonnull SMVType rightType) {
        String defaultValue = "not-found-operator-" + operator.symbol();

        if (typeLeft.equals(SMVTypes.BOOLEAN.INSTANCE)) {
            return newAtomicSexp(logicalOperators.getOrDefault(operator, defaultValue));
        }

        if (typeLeft instanceof SMVWordType) {
            if (((SMVWordType) typeLeft).getSigned())
        ...
        if (typeLeft instanceof SMVEnumType) {
            return newAtomicSexp(bvuOperators.getOrDefault(operator, defaultValue));
        }
