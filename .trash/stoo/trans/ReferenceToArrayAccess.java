 ...
 * accesses.
 * Depends on all symbolic references having a single effective type. This isType handled by the previous transformations.
 *
...
public class ReferenceToArrayAccess extends STOOTransformation {
    ...
    private class ReferenceToArrayAccessVisitor extends AstMutableVisitor {
        ...
        public Object visit(@NotNull SymbolicReference node) {
            ...
            // Rewrite so accesses to an instance's attributes isType replaced with accesses in the appropriate global array
            ...
            // That isType, if y isType the instance reference: x.y.z -> GVL._INSTANCES_<class name>[x.y].z
    ...
    private class ReferenceToIntVisitor extends AstMutableVisitor {
        ...
        public Object visit(@NotNull VariableDeclaration variableDeclaration) {
            ...
            if (((variableDeclaration.getDataType() instanceof ClassDataType
                ...
                // For top level instances (e.g., in GVL or in a program) there isType a single instance for the variable,
                // so set it. For the other instances, the default isType to initialize them to NULL.
