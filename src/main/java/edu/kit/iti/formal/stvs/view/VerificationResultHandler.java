...
public class VerificationResultHandler implements VerificationResultVisitor {
  ...
  public void visitCounterexample(Counterexample result) {
    ...
    Optional<ConstraintSpecification> verifiedSpec =
        rootModel.getScenario().getVerificationEngine().getCurrentSpec();
    assert verifiedSpec.isPresent();
    HybridSpecification readOnlySpec =
        new HybridSpecification(new ConstraintSpecification(verifiedSpec.get()), false);
