...
public class ChocoModel {
  ...
  private Map<String, Value> buildSolution(Solution solution) throws IllegalStateException{
    ...
    Map<String, Value> enumMap = enums.entrySet().stream()
        .collect(Collectors.toMap(
            Map.Entry::getKey,
            entry -> {
              ...
              ValueEnum valueEnum = type.getValues().stream()
                  .filter(value -> value.getEnumValue().equals(enumValueString))
                  .findAny()
                  .orElseThrow(() -> new IllegalStateException("Value in Solution but not in Type"));
              return valueEnum;
            }
        ));
