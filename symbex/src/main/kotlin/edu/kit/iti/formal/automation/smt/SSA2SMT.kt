...
class SSA2SMT(val input: SMVModule) : Runnable {
    ...
    override fun run() {
        ...
        input.definitions.forEach { (k, expr) ->
            product.nextDefines[k.name] = expr.accept(v)
        }
...
class Smv2SmtVisitor(
    val fnTranslator: S2SFunctionTranslator,
    val dtTranslator: S2SDataTypeTranslator,
    val statePrefix: String = SMTProgram.STATE_NAME,
) : SMVAstVisitor<SExpr> {
    override fun visit(top: SMVAst): SExpr = throw IllegalStateException("illegal AST node discovered!")

    override fun visit(v: SVariable): SExpr = SSymbol(statePrefix + v.name)

    override fun visit(be: SBinaryExpression): SExpr {
        val left = be.left.accept(this)
        val right = be.right.accept(this)
        val op = fnTranslator.translateOperator(
            be.operator,
            be.left.dataType,
            be.right.dataType,
        )
    ...
    override fun visit(l: SLiteral): SExpr = dtTranslator.translate(l)

    override fun visit(a: SAssignment): SExpr = throw IllegalStateException("illegal AST node discovered!")

    override fun visit(ce: SCaseExpression): SExpr = ifThenElse(ce.cases, 0)
